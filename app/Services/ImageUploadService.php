...
{
    const ORIGINAL_FILE_PATH = '/storage/original-pictures/';
    ...
    private function uploadOriginalFile(ImageRequest $path): string
    {
        ...
        if (!is_dir(__DIR__ . '/../..' . self::ORIGINAL_FILE_PATH . $path->getId())) {
            mkdir(__DIR__ . '/../..' . self::ORIGINAL_FILE_PATH . $path->getId(), 0777, true);
        }
        $originalFileLocation = __DIR__ . '/../..' . self::ORIGINAL_FILE_PATH . $path->getId() . '/' .
    ...
    private function uploadPreviewFile(ImageRequest $path, string $originalFileLocation): string
    {
        ...
        if (!is_dir(__DIR__ . '/../..' . self::PREVIEW_FILE_PATH . $path->getId())) {
            mkdir(__DIR__ . '/../..' . self::PREVIEW_FILE_PATH . $path->getId(), 0777, true);
        }

        $previewFileLocation = __DIR__ . '/../..' . self::PREVIEW_FILE_PATH . $path->getId() . '/' . $previewFileName;
    ...
    private function saveUploadedFiles(ImageRequest $path, string $previewFileName): void
    {
        ...
        $this->imageRepository->edit(
            $path, self::PREVIEW_FILE_PATH . $path->getId() . '/' . $previewFileName, '_preview');
        $this->imageRepository->edit(
            $path, self::ORIGINAL_FILE_PATH . $path->getId() . '/' . $originalFileName . '.' .
